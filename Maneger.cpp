...
int main()
{
	...
	std::function<void(int)> func( // any function here to be executed with threads n_data times
		[](int i) noexcept {std::cout << i << std::endl; }
	);
	OPTManeger<std::function<void(int)>> 
		man(
			4, // n_threads
			50, // n_data
			func // function
...
OPTManeger<Fn_type>::OPTManeger(const int n_threads, const int n_data, Fn_type& in_fn) :
	...
	data_idx(0),
	threads(),
	write_lock(), finish_lock(), start_lock(),
	c_v(),
...
void OPTManeger<Fn_type>::init()
{
	...
	sync_begin = new opt::gates<std::function<void()>>(N_THREADS, begin);
	...
	sync_finish = new opt::gates<std::function<void()>>(N_THREADS, complete);
			...
			std::thread(
				[&, i]() mutable noexcept
				{
					...
					while (true) // running loop
					{
						...
						{
							write_lock.lock(); // lock before getting data index
							thread_data_idx = data_idx++; // read and increment index for next thread
							write_lock.unlock(); // no more mutations of shared variables
